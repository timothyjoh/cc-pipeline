...
export async function runEngine(projectDir, options = {}) {
    const quiet = !!options.quiet; // suppress banner/step logs when TUI is active
    const log = (...args) => { if (!quiet)
        console.log(...args); };
    const logErr = (...args) => { if (!quiet)
        console.error(...args); };
    ...
    const handleSignal = (signal) => {
        if (interrupted)
            return; // Already handling a signal, ignore duplicates
        ...
        while (phase <= maxPhases) {
            ...
            for (let i = stepIndex; i < config.steps.length; i++) {
                ...
                for (let attempt = 0; attempt < retryDelays.length; attempt++) {
                    ...
                    if (stepDef.agent === 'bash' || stepDef.continueOnError)
                        break;
                    // Log retry
 ...
 */
async function runStep(phase, stepDef, projectDir, config, logFile, options = {}, sessionCostUSD = 0) {
    const { name: stepName, agent, skipUnless, output, testGate } = stepDef;
    const log = (...args) => { if (!options.quiet)
        console.log(...args); };
    ...
    const stepCostUSD = result.usage?.costUSD ?? 0;
    // Log step done
    ...
    if (status === 'error') {
        ...
        if (!description && result.outputPath) {
            try {
                ...
                if (raw)
                    description = raw.slice(-2000); // last 2000 chars to stay concise
            }
            catch (_) { }
        }
    }
