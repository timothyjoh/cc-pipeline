...
export class CodexAgent extends BaseAgent {
    async run(phase, step, promptPath, model, context) {
        ...
        for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
            ...
            if (attempt > 1) {
                ...
                try {
                    appendFileSync(outputPath, sep, 'utf-8');
                }
                catch (_) { }
            }
            ...
            try {
                appendFileSync(outputPath, msg, 'utf-8');
            }
            catch (_) { }
        }
    ...
    spawnOnce(args, projectDir, outputPath) {
        return new Promise((resolve) => {
            ...
            const onData = (chunk) => {
                ...
                try {
                    appendFileSync(outputPath, chunk.toString(), 'utf-8');
                }
                catch (_) { }
            };
