    ...
    try {
        ...
        const engineStarted = new Promise((resolve) => {
            child.stdout.on('data', (data) => {
                ...
                if (stdout.includes('Running step:'))
                    resolve();
            });
        });
        ...
        if (exitCode === 130 && hasStepStartEvent) {
            assert.strictEqual(hasInterruptedEvent, true, 'Should have interrupted event when process caught SIGINT mid-step');
        }
...
test('signal handling: process exits within timeout after SIGINT', async () => {
    ...
    try {
        ...
        const sigEngineStarted = new Promise((resolve) => {
            child.stdout.on('data', (data) => {
                sigStdout += data.toString();
                if (sigStdout.includes('Running step:'))
                    resolve();
            });
        });
