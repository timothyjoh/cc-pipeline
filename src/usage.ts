 ...
 */
export function computeUsagePercentages(
  sessionCostUSD: number,
  usageLimits: { sessionBudgetUSD: number; weeklyBudgetUSD: number },
): { session_percentage: number; weekly_percentage: number } {
  const weeklyCostUSD = getWeeklyCostUSD();

  const session_percentage = parseFloat(
    ((sessionCostUSD / usageLimits.sessionBudgetUSD) * 100).toFixed(1),
  );
  const weekly_percentage = parseFloat(
    ((weeklyCostUSD / usageLimits.weeklyBudgetUSD) * 100).toFixed(1),
  );
